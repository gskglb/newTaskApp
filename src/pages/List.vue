<template>
  <q-page class="bg-grey-10" style="margin-top:0px">
    ...
    <q-list bordered dark class="rounded-borders q-ma-sm"  v-for="(record) in tasksList" v-bind:key="record.keyRef">
      <q-item-label header>{{record.start_date_time  | formatDate}}</q-item-label>
      <q-item>
          <q-item-section top>
            <q-item-label lines="1">
              <span class="text-weight-medium">{{record.title | limitText}}</span>
          ...
          <q-item-section top side>
            <div class="q-gutter-xs">
              <q-btn size="12px" flat icon="more_vert" @click="showOptions(record)"/>
            </div>
          </q-item-section>
      </q-item>
      <q-item>
  ...
  methods: {
    ...
    showOptions (record) {
      this.$q.bottomSheet({
        ...
        if (action.id === 'edit') {
          this.editTaskFn(record)
        } else if (action.id === 'delete') {
          this.deleteTask(record)
        } else if (action.id === 'add_notes') {
          this.addNotesFn(record)
        }
      }).onCancel(() => {
